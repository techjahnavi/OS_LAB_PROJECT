...
function run_algo() {
	...
	var q = 1;
	for (var j = 1; j <= 5; j++) {
		x1 = parseInt(document.getElementById("av11").value); //av is for avaible
		x2 = parseInt(document.getElementById("av12").value);
		x3 = parseInt(document.getElementById("av13").value);
		for (var i = k; i <= 5; i++) {
			//5 is because we have taken 5 processses
			var ex1 = parseInt(document.getElementById("a" + i + "1").value); //a is for alocation
			...
			if (ex1 != 0 || ex2 != 0 || ex3 != 0) {
				if (
					...
					//n is for needed
					document.getElementById("p" + q).value = "P" + i; //display the process
					document.getElementById("av11").value =
						parseInt(document.getElementById("av11").value) +
						parseInt(document.getElementById("a" + i + "1").value); //updation of avaible
					document.getElementById("av12").value =
